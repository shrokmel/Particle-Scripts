...
def plot_frames(beads, sim, ti, tf, save_eps):
    ...
    savebase += 'density_' + str(sim.density) + '_kappa_' + str(sim.kappa) + \
        '_km_' + str(sim.km) + '_panti_' + str(sim.pa) + '/'
    os.system("mkdir -p " + savebase)
    quant_steps = 2056
    norm = mpl.colors.Normalize(vmin=0, vmax=sim.nfils)  
    downlim = -10
    uplim = lx+10
    num_ticks = 5
    ...
    for step in range(ti, tf):
        ...
        line0 = ax0.scatter(x, y, s=1, c=beads.cid, \
                            cmap=plt.cm.get_cmap('jet',quant_steps), \
                            edgecolors='None', alpha=0.7, vmin=0, vmax=sim.nfils, \
...
def main():
    ...
    print args.folder, sim.lx, sim.ly
